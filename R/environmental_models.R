...
fl_map<-st_read("Data/Spatial_Data/Detailed_Florida_State_Boundary/Detailed_Florida_State_Boundary.shp")
...
#load ant traits
full_Set1<-readRDS("Data/Trait_Data/trait_clean.rds")
...
write.csv(AICQWD_native, "R/SDMTMB_models/AIC_tables//AICQWD_native.csv")
...
write.csv(AICCS_native, "R/SDMTMB_models/AIC_tables/AIC_CS_native.csv")
...
m1<-sdmTMB::sdmTMB(data = gps_coordsnn2, mesh = mesh, (QWD) ~ scale(ATR),
                   spatial = "on",family = Gamma(link = 'log'))
saveRDS(m1, "R/SDMTMB_models/QWD_nonnative/QWD_ATR_sdmtmb_nonnative.rds")

m2<-sdmTMB::sdmTMB(data = gps_coordsnn2, mesh = mesh, (QWD) ~ scale(MAT) ,
                   spatial = "on",family = Gamma(link = 'log'))
saveRDS(m2, "R/SDMTMB_models/QWD_nonnative/QWD_MAT_sdmtmb_nonnative.rds")

m3<-sdmTMB::sdmTMB(data = gps_coordsnn2, mesh = mesh, (QWD) ~ scale(MAP),
                   spatial = "on",family = Gamma(link = 'log'))
saveRDS(m3, "R/SDMTMB_models/QWD_nonnative/QWD_MAP_sdmtmb_nonnative.rds")

nullmod<-sdmTMB::sdmTMB(data = gps_coordsnn2, mesh = mesh, (QWD) ~ 1,
                        spatial = "off",family = Gamma(link = 'log'))
saveRDS(nullmod, "R/SDMTMB_models/QWD_nonnative/QWD_nullmod_sdmtmb_nonnative.rds")
...
AICQWD_nonnative<-AICtab(m1,m2,m3,nullmod,
                      weights = T, delta = T, base = TRUE, sort = TRUE)
write.csv(AICQWD_nonnative,"R/SDMTMB_models/AIC_tables/AICQWD_nonnative.csv")
...
cs1<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, size ~ scale(ATR),
                    spatial = "on", family = poisson(link = 'log'))
saveRDS(cs1, "R/SDMTMB_models/Size_nonnative/CS_poisson_ATR_sdmtmb_native.rds")

cs1nb<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, (size) ~ scale(ATR),
                      spatial = "on",family = nbinom2())
saveRDS(cs1nb, "R/SDMTMB_models/Size_nonnative/CS_nb_ATR_sdmtmb_native.rds")

cs2<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, (size) ~ scale(MAT),
                    spatial = "on",family = poisson(link = 'log'))
saveRDS(cs2, "R/SDMTMB_models/Size_nonnative/CS_poisson_MAT_sdmtmb_native.rds")

cs2nb<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, (size) ~ scale(MAT),
                      spatial = "on",family = nbinom2())
saveRDS(cs2nb, "R/SDMTMB_models/Size_nonnative/CS_nb_MAT_sdmtmb_native.rds")

cs3<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, (size) ~ scale(MAP),
                    spatial = "on",family = poisson(link = 'log'))
saveRDS(cs3, "R/SDMTMB_models/Size_nonnative/CS_poisson_MAP_sdmtmb_native.rds")

cs3nb<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, (size) ~ scale(MAP),
                      spatial = "on",family = nbinom2())
saveRDS(cs3nb, "R/SDMTMB_models/Size_nonnative/CS_nb_MAP_sdmtmb_native.rds")

nullmod<-sdmTMB::sdmTMB(data = gps_coords2nn, mesh = mesh, (size) ~ 1,
                        spatial = "off",family = poisson(link = 'log'))
saveRDS(nullmod, "R/SDMTMB_models/Size_nonnative/CS_nullmod_sdmtmb_native.rds")


CS_nonnative_AIC<-AICtab(cs1,cs1nb, cs2,cs2nb,cs3,cs3nb,nullmod, weights = T,
       delta = T, base = T)
write.csv(CS_nonnative_AIC, "R/SDMTMB_models/AIC_tables/AIC_CS_nonnative.csv")
...
r_pois <- DHARMa::createDHARMa(
  simulatedResponse = s_gamma,
  observedResponse = gps_coords2nn$size,
  fittedPredictedResponse = pred_fixed
)
plot(r_pois)
