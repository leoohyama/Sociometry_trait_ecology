...
morph<-df %>%
  dplyr::select(Genus.Species, QWD, diet) %>%
  drop_na()
...
#check if data is all there
sort(setdiff(new_morph_df2$Genus.Species, pruned_FLtree$tip.label))
#species in the phylogeny but not in the data
sort(setdiff(pruned_FLtree$tip.label, new_morph_df2$Genus.Species))
...
################
################
log(new_morph_df2$QWD)
spp<-new_morph_df2$Genus.Species

bm<-corBrownian(1,phy = pruned_FLtree, form = ~spp)
pl<-corPagel(1,phy = pruned_FLtree, form = ~spp)

gls1<-gls(log(QWD) ~ diet,
          data = new_morph_df2,
          correlation = bm)

gls2<-gls(data = new_morph_df2,
          log(QWD) ~ diet,
          correlation = pl)
...
get_model_vals_QWDr<-function(x, directory){
  setwd(directory)
  pairs1<-emmeans(gls2, specs = "diet", type = "response")
...
dir<-"/Users/leoohyama/Google Drive/Colony_Size_FL_Project/FL_colony_size_proj/R/PGLS/QWD_as_response/Predictor_diet/QWD_diet_model_summary/"
get_model_vals_QWDr(gls2, directory = dir)
