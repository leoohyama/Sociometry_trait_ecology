...
morph<-df %>%
  dplyr::select(Genus.Species, QWD_ML, diet) %>%
  drop_na()
...
gls1<-gls(sqrt(QWD_ML) ~ diet,data= new_morph_df2,
          correlation = bm)

gls2<-gls(sqrt(QWD_ML) ~ diet,data= new_morph_df2,
          correlation = bm1)
...
dir<-"/Users/leoohyama/Google Drive/Colony_Size_FL_Project/FL_colony_size_proj/R/PGLS/QWD_as_response/Predictor_diet/QWD_ML_ diet_model_summary/"
get_model_vals_QWDr(gls2,
                    directory = dir,
                    spec = "diet")
