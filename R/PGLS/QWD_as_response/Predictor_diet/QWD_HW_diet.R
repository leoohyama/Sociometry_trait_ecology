...
morph<-df %>%
  dplyr::select(Genus.Species, QWD_HW, diet) %>%
  drop_na()
...
bm1
gls1<-gls((QWD_HW) ~ diet,data= new_morph_df2,
          correlation = bm)
gls2<-gls((QWD_HW) ~ diet,data= new_morph_df2,
          correlation = bm1)
...
get_model_vals_QWDr<-function(x, directory, spec){
  ...
  write.csv(as.data.frame(pairs1), "Emmeans_output_hw.csv")
  write.csv(as.data.frame(pairs(pairs1)), "Emmeans_pairs_output_hw.csv")
  r2<-rr2::R2(gls2)
  write.csv(as.data.frame(r2), "R2.csv")

}
dir<-"/Users/leoohyama/Google Drive/Colony_Size_FL_Project/FL_colony_size_proj/R/PGLS/QWD_as_response/Predictor_diet/QWD_HW_diet_model_summary/"
