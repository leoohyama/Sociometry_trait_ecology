...
morph<-df %>%
  dplyr::select(Genus.Species, QWD_HW, nesting_strata) %>%
  drop_na()
...
bm
gls1<-gls(log(QWD_HW+34) ~ nesting_strata,data= new_morph_df2,
          correlation = bm)

bm1<-corPagel(1, pruned_FLtree, form = ~ordercc)
bm1
gls2<-gls(log(QWD_HW+34) ~ nesting_strata,data= new_morph_df2,
          correlation = bm1)
...
get_model_vals_QWDr<-function(x, directory, spec){
  ...
  write.csv(as.data.frame(pairs1), "Emmeans_output_QWD_HW.csv")
  write.csv(as.data.frame(pairs(pairs1)), "Emmeans_pairs_output_QWD_HW.csv")
  r2<-rr2::R2(gls2)
  write.csv(as.data.frame(r2), "R2.csv")

}
dir<-"/Users/leoohyama/Google Drive/Colony_Size_FL_Project/FL_colony_size_proj/R/PGLS/QWD_as_response/Predictor_nest/QWD_HW~nest_model_summary/"
