...
#PGLS for colony against native
...
morph<-df %>%
  dplyr::select(Genus.Species, median_size, status) %>%
  drop_na()
...
bm
gls1<-gls(log(size) ~ status,data= new_morph_df2,
          correlation = bm)

bm1<-corPagel(1, pruned_FLtree, form = ~ordercc)
bm1
gls2<-gls(log(size) ~ status,data= new_morph_df2,
          correlation = bm1)
...
get_model_vals_QWDst<-function(x, directory){
  setwd(directory)
  pairs1<-emmeans(gls2, specs = "status", type = "response")

  write.csv(as.data.frame(pairs1), "Emmeans_output_colony_size_status.csv")


  write.csv(as.data.frame(pairs(pairs1)), "Emmeans_pairs_output_colony_size_status.csv")
  r2<-rr2::R2(gls2)
  write.csv(as.data.frame(r2), "R2.csv")

}

dir<-"/Users/leoohyama/Google Drive/Colony_Size_FL_Project/FL_colony_size_proj/R/PGLS/Colony_size_as_response/Colony~status_summary/"
get_model_vals_QWDst(gls2, directory = dir)
